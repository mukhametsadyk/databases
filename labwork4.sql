...
-- Task 1.4
SELECT first_name || ' ' || last_name AS name, COALESCE(email, 'No email provided') AS email FROM employees;
-- Task 2.1
...
SUBSTRING(email FROM 1 FOR 3) AS email_part
FROM employees;
...
-- Task 3.3
SELECT FORMAT('Project: %s - Budget: $%s - Status: %s', project_name, budget, status) AS info
FROM projects;
...
FROM employees;
-- Task 5.1
SELECT employee_id, first_name || ' ' || last_name AS name, salary FROM employees WHERE salary > 65000
UNION
...
EXCEPT
SELECT e.* FROM employees e JOIN assignments a ON e.employee_id = a.employee_id;
-- Task 6.1
...
-- Task 7.2
SELECT p.project_name, SUM(a.hours_worked) AS total_hours, COUNT(DISTINCT a.employee_id) AS emp_count
FROM projects p
...
MAX(first_name || ' ' || last_name) AS top_employee,
GREATEST(MAX(salary), MIN(salary)) AS great_val,
LEAST(MAX(salary), MIN(salary)) AS low_val
FROM employees
GROUP BY department;
